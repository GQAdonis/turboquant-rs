...
<task type="auto">
  ...
  <files>src/polar_quant.rs, src/kv_cache.rs, src/turboquant.rs, src/rotation.rs</files>
  <read_first>
    - src/polar_quant.rs (existing batch_quantize, batch_inner_product)
    - src/kv_cache.rs (existing batch_attend)
    - src/turboquant.rs (TurboQuant struct, mse field)
    - src/rotation.rs (Rotation struct, signs field)
  ...
  <action>
1. **src/rotation.rs** -- Add `signs()` accessor to `Rotation<B>` if it does not already exist:
   ```rust
   pub fn signs(&self) -> &[i8] { &self.signs }
   ```

2. **src/turboquant.rs** -- Add `mse_polar()` accessor to `TurboQuant<B>` to expose the MSE PolarQuant for direct batch operations:
   ```rust
   /// Access the MSE PolarQuant (Stage 1) for direct batch operations.
   pub fn mse_polar(&self) -> &PolarQuant<B> { &self.mse }
   ```

3. **src/polar_quant.rs** -- Add GPU-specific impl block behind `#[cfg(feature = "gpu")]`:
   ...
   Add a `PolarQuant<GpuBackend>` impl block with four methods -- two private GPU kernel methods and two public dispatch methods:
...
4. **src/kv_cache.rs** -- Add GPU-aware attend dispatch:
   ...
   Add `KvCache<GpuBackend>` impl block:
   ```rust
   #[cfg(feature = "gpu")]
  ...
  <acceptance_criteria>
    - src/rotation.rs contains `pub fn signs(&self) -> &[i8]`
  ...
  <done>GPU dispatch methods on PolarQuant<GpuBackend> and KvCache<GpuBackend> route large batches (>=32) to GPU kernels and small batches to CPU rayon. Accessors added to Rotation (signs) and TurboQuant (mse_polar). All behind #[cfg(feature = "gpu")].</done>
</task>
